...
- 例外：
  ...
  - 原文是 `{{内容}}`、译文是 `{{内容}}}` 这种多一个花括号的情况也不算错误
...
```json
[
  { "id": "R1", "criteria": "译文有明显输入错误，如错别字、漏字、多字或误输入字符。花括号 {} 不算错误，不需要对称，可随意插入，相当于空格。原文是 {{内容}}、译文是 {{内容}}} 这种多一个花括号的情况也不算错误。", "report": "命中时只报告明确的输入错误。花括号 {} 本身不算问题，不需要对称，可随意插入，相当于空格。原文是 {{内容}}、译文是 {{内容}}} 这种多一个花括号的情况也不算错误。reason 需要详细完整说明具体错误点" },
