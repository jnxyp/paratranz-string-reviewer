...
```json
[
  { "id": "R1", "criteria": "译文有明显输入错误，如错别字、漏字、多字或误输入字符。花括号 {} 不算错误，不需要对称，可随意插入，相当于空格。", "report": "命中时只报告明确的输入错误。花括号 {} 本身不算问题，不需要对称，可随意插入，相当于空格。reason 需要详细完整说明具体错误点" },
  { "id": "R2", "criteria": "译文存在明显叠字或重复片段", "report": "命中时需要详细完整说明重复了什么内容，以及为什么可以判断为误重复" },
